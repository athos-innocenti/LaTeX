...
Riassumendo, per entrambi le tipologie di MOSFET si identificano tre regioni operative:
\begin{enumerate}
    \item \textit{Cutoff}: non c'è conduzione tra source e drain perché $|V_{gs}| < |V_{t}|$ implica che non può scorrere corrente all'interno del canale
    \item \textit{Regione lineare}: in questo caso $|V_{gs}| > |V_{t}|$ e $|I_{d}| < |I_{d-sat}|$. Finché non si raggiunge la corrente limite $I_{d-sat}$ dettata dalla tensione $V_{gs}$, si ha un andamento inizialmente lineare e poi più marcatamente parabolico della corrente. In queste condizioni il dispositivo si comporta come un resistore controllato in tensione
    \item \textit{Saturazione}: in questo caso $|I_{d}| = |I_{d-sat}|$. Si è raggiunto il valore limite della corrente definito tramite l'equazione \eqref{id_sat}. La corrente non cresce più, quindi l'uscita è costante indipendentemente dalla tensione applicata sul drain. In queste condizioni il dispositivo si comporta come un generatore di corrente costante controllato in tensione
\end{enumerate}
...
\begin{itemize}
    \item\textbf{NOT}: indicato con una linea sopra l'operando che deve essere negato
    \begin{figure}[h]
        \centering
        \includegraphics[scale=0.4,angle=0]{logica_not.png}
    ...
    %     1 &0
    %\end{tabular}
    \item\textbf{AND}: indicato come il prodotto tra i due operandi
    \begin{figure}[h]
        \centering
        \includegraphics[scale=0.4,angle=0]{logica_and.png}
        \caption{Simbolo circuitale dell' AND}
    \end{figure}
    \item\textbf{OR}: indicato come la somma tra i due operandi
    \begin{figure}[h]
        \centering
        \includegraphics[scale=0.4,angle=0]{logica_or.png}
