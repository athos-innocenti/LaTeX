...
\section{NOT}
La porta NOT è la più semplice porta da realizzare perché sono sufficienti due resistenze e un solo transistor. La sua versione basilare in logica RTL è data dal seguente circuito:
\begin{figure}[h]
    \centering
...
La corrente che passa attraverso la resistenza di collettore può passare sia attraverso $Q_1$ che $Q_2$, quindi è sufficiente che una delle due celle sia attiva (ovvero basta che almeno una delle due tensioni in ingresso $V_{in-a}$ e $V_{in-b}$ sia \textit{alta}) per far scorrere una corrente in $R_{c}$ e avere uscita \textit{OUT bassa} ricalcando esattamente la tabella di verità del NOR.
