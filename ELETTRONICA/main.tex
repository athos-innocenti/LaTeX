...
\section{N-MOS}
Un \textbf{N-MOS} è anche chiamato \textbf{mosfet a canale n}.
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.45,angle=0]{n_mos_simbolo.png}
...
\begin{itemize}
    ...
    \item[$V_{gs}$]: la tensione tra gain e source. È \textit{positiva} quando gate è positivo rispetto a source, cioè quando $V_{g} > V_{s}$
...
\begin{itemize}
    ...
    \item[$I_{s}$]: la corrente del source, si prende \textit{positiva} quando è \textit{uscente} nel dispositivo
\end{itemize}
La particolarità dell' N-MOS è che la corrente del gate $I_{g}$ è \textit{sempre nulla} in tensione \textit{continua}.
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.4,angle=0]{n_mos_struttura.png}
    \caption{Struttura di un N-MOS}
\end{figure}
\subsection{Regioni operative}
\section{P-MOS}
Anche chiamato \textbf{mosfet a canale p}
