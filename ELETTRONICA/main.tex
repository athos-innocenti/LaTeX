...
La corrente che passa attraverso la resistenza di collettore può passare sia attraverso $Q_1$ che $Q_2$, quindi è sufficiente che una delle due celle sia attiva (cioè basta che almeno una delle due tensioni in ingresso $V_{in-a}$ e $V_{in-b}$ sia \textit{alta}) per far scorrere una corrente in $R_{c}$ e avere uscita \textit{OUT bassa} ricalcando esattamente la tabella di verità del NOR.
